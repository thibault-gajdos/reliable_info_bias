...
# Master list used for ordering. Individual models contain only a subset.
...
# Identify the model from its filename. The order is important because
# "learning_localeta" also contains the shorter string "localeta".
get_model_spec <- function(filename) {
  ...
  if (str_detect(filename_lower, "learning_localeta")) {
    return(list(
      ...
      parameters = c("mu_alpha", "mu_beta", "mu_lambda", "mu_delta", "mu_eta")
    ))
  }
  ...
  if (str_detect(filename_lower, "localeta")) {
    return(list(
      model = "Local eta",
      parameters = c("mu_alpha", "mu_beta", "mu_lambda", "mu_eta")
    ))
  }
  ...
  if (str_detect(filename_lower, "learning")) {
    return(list(
      model = "Learning",
      parameters = c("mu_alpha", "mu_beta", "mu_lambda", "mu_delta")
    ))
  }
  ...
  if (str_detect(filename_lower, "simple")) {
    return(list(
      model = "Simple",
      parameters = c("mu_alpha", "mu_beta", "mu_lambda")
    ))
  }

  stop("Could not identify model type from filename: ", filename)
}

fits <- set_names(map(unname(fit_files), load_fit), names(fit_files))

model_specs <- map(fit_files, get_model_spec)
model_types <- unique(map_chr(model_specs, "model"))
...
cat("Loaded ", model_types, " fits for all five groups.\n", sep = "")
...
extract_group_draws <- function(fit, group_name, alpha_scale = 6, delta_scale = 2) {
  ...
  filename <- unname(fit_files[[group_name]])
  spec <- get_model_spec(filename)
  expected_parameters <- spec$parameters
  
  # Preferred route: use transformed generated quantities saved by Stan.
  if (all(expected_parameters %in% names(d))) {
    out <- d %>% select(all_of(expected_parameters))
  } else {
    # Fallback route: transform the raw group-level mu_pr parameters.
    n_parameters <- length(expected_parameters)
    bracket <- paste0("mu_pr[", seq_len(n_parameters), "]")
    dotted  <- paste0("mu_pr.", seq_len(n_parameters), ".")
    ...
    if (all(bracket %in% names(d))) {
      ...
      raw_names <- raw_names[seq_len(min(n_parameters, length(raw_names)))]
    }

    if (length(raw_names) < n_parameters) {
      stop(
        "Could not find the expected group-level parameters for ", group_name,
        "\nModel identified from filename: ", spec$model,
        "\nExpected: ", paste(expected_parameters, collapse = ", "),
        "\nAvailable matching names: ",
        paste(names(d)[str_detect(names(d), "mu_|alpha|beta|lambda|delta|eta")],
              collapse = ", ")
      )
    }

    out <- d %>% select(all_of(raw_names[seq_len(n_parameters)]))
    names(out) <- expected_parameters

    out$mu_alpha <- pnorm(out$mu_alpha) * alpha_scale
    out$mu_lambda <- pnorm(out$mu_lambda)

    if ("mu_delta" %in% expected_parameters) {
      out$mu_delta <- pnorm(out$mu_delta) * delta_scale
    }
  }
...
draws_long <- draws_wide %>%
  pivot_longer(any_of(parameter_names), names_to = "parameter", values_to = "value") %>%
...
active_parameter_names <- parameter_names[parameter_names %in% unique(draws_long$parameter)]

stopifnot(
  all(is.finite(draws_long$value)),
  n_distinct(draws_long$group) == 5L,
  length(active_parameter_names) == length(model_specs[[1]]$parameters)
)

cat("\nParameters found in the selected model:\n")
print(unname(parameter_labels[active_parameter_names]))
...
p_distributions <- ggplot(draws_long, aes(value, fill = group, colour = group)) +
  ...
  labs(
    ...
    subtitle = paste("Five groups,", model_types, "model"),
    x = "Parameter value",
    y = "Density",
    fill = NULL,
    colour = NULL
...
write_csv(
  distortion_dots_df,
  file.path(table_dir, "distorted_reliability_50_55_65.csv")
)
...
available_delta_eta <- intersect(c("mu_delta", "mu_eta"), active_parameter_names)
...
reference_lines <- tibble(
  ...
  parameter_clean = factor(
    unname(parameter_labels[available_delta_eta]),
    levels = c("delta", "eta")
  ),
  reference = unname(reference_values[available_delta_eta])
)
...
delta_eta_filename <- if (identical(available_delta_eta, "mu_eta")) {
  "eta_posterior_five_groups"
} else if (identical(available_delta_eta, "mu_delta")) {
  "delta_posterior_five_groups"
} else {
  "delta_eta_posteriors_five_groups"
}

save_plot_both(p_delta_eta, delta_eta_filename, 12, 5.5)
...
reference_stats <- draws_long %>%
  ...
  summarise(
    ...
    l95 = quantile(value, .025),
    u95 = quantile(value, .975),
    .groups = "drop"
  ) %>%
  mutate(
    credible_interval_excludes_reference =
      l95 > reference | u95 < reference
  )
...
posterior_pairwise <- map_dfr(active_parameter_names, function(parameter_name) {
...
extract_participant_params <- function(fit, group_name) {
  filename <- unname(fit_files[[group_name]])
  spec <- get_model_spec(filename)
  participant_parameter_names <- unname(parameter_labels[spec$parameters])
  ...
  out <- tibble(
    ...
    filter(
      ...
      parameter_index %in% seq_along(participant_parameter_names)
    ) %>%
    mutate(
      parameter = participant_parameter_names[parameter_index],
      group = group_name,
...
if (RUN_PARTICIPANT_TESTS) {
  ...
  set.seed(SEED)
  active_parameter_clean <- unname(parameter_labels[active_parameter_names])

  randomisation_stats <- map_dfr(active_parameter_clean,

functions {
  ...
  real partial_sum(array[] int slice_indices,
    ...
    for (i in 1:size(slice_indices)) {
      ...
      params[8] = Phi_approx(mu_pr[8] + sigma_pr[8] * param_raw[n, 8]);// rho is the initial probability of blue, between 0 and 1
      // rho < 0.5: red bias; rho = 0.5: neutral; rho > 0.5: blue bias
      
 
      real beliefcount_blue = 2.0 * params[8];
      real beliefcount_red  = 2.0 * (1.0 - params[8]);
...
generated quantities {
    ...
    real mu_rho = Phi_approx(mu_pr[8]);
    ...
    for (n in 1:N) {
        ...
        params[n, 8] = Phi_approx(mu_pr[8] + sigma_pr[8] * param_raw[n, 8]);

        real beliefcount_blue = 2.0 * params[n, 8];
        real beliefcount_red  = 2.0 * (1.0 - params[n, 8]);

...
# PARAMETER RECOVERY — GLOBAL ETA MODEL, EXP11 AWARE


# This script runs parameter recovery for the Exp11 Implicit Aware version of
# the sequential choice model with global eta. Eta is applied once to the
# complete set of reliability- and recency-weighted samples on a trial. The
# learned prior is added only after eta has scaled the combined sample evidence.
#
...
# - eta: one global modulation of the complete sample evidence on each trial
#
#
# Feedback coding:
#
...
# This script is for the Implicit Aware Base Rate group.
# The grid values are taken directly from the Exp11 Aware global-eta posterior:
#
#   alpha  = 2.6105562 [1.6370368, 3.7184372]
#   beta   = 0.3412604 [0.1740596, 0.5060431]
#   lambda = 0.0574026 [0.0284014, 0.0890891]
#   delta  = 0.8665815 [0.6181856, 1.0938314]
#   eta    = -0.3450125 [-0.7318050, 0.0526856]
#
...
# Run this script from the reliable_info_bias project root on Apocrita.
...
if (length(args) != 1L || is.na(suppressWarnings(as.integer(args[1])))) {
  stop("Supply one integer grid index: Rscript recover_choice_eta_global_aware.R <k>")
}

k <- as.integer(args[1])

if (k < 1L || k > 243L) {
  stop("Grid index k must be between 1 and 243.")
}

results_dir <- file.path(project_dir, "results", "choice_eta_global_aware_exp11")
dir.create(results_dir, recursive = TRUE, showWarnings = FALSE)

out_file <- file.path(results_dir, sprintf("recover_%d.rds", k))
...
stan_file <- file.path(project_dir, "stan", "log_trunc_global_eta_model.stan")

if (!file.exists(stan_file)) {
  stop("Global-eta Stan file not found: ", stan_file)
}
...
#
# This learning model uses the Implicit Aware prior-belief data because it needs
...
data_file <- file.path(project_dir, "data", "data_priorbelief_aware_exp11.rdata")

if (!file.exists(data_file)) {
  stop("Exp11 Aware data file not found: ", data_file)
}

load(file = data_file)

if (!exists("data") || !is.data.frame(data)) {
  stop("The data file did not create a data.frame called 'data'.")
}

# The Exp11 data have appeared with either of these column names. Use the
# Manipulation_ version when present, matching the global-eta fitting script.
if ("Manipulation_ResponseButtonOrder" %in% names(data)) {
  data$RBO_recovery <- data$Manipulation_ResponseButtonOrder
} else if ("ResponseButtonOrder" %in% names(data)) {
  data$RBO_recovery <- data$ResponseButtonOrder
} else {
  stop("No response-button-order column was found.")
}
...
data <- data %>%
  ...
  mutate(
    choice = case_when(
      RBO_recovery == 1 & Response == 0 ~ 2,
      RBO_recovery == 1 & Response == 1 ~ 1,
      RBO_recovery == 0 & Response == 0 ~ 1,
      RBO_recovery == 0 & Response == 1 ~ 2,
      TRUE ~ NA_real_
    )
  ) %>%
  mutate(
  ...
  ungroup() %>%
  mutate(
    # CorrectResponse is button-coded, so convert it to the objectively
    # correct colour: 1 = Blue and 0 = Red.
    feedback = case_when(
      RBO_recovery == 1 & CorrectResponse == 1 ~ 1L,
      RBO_recovery == 1 & CorrectResponse == 0 ~ 0L,
      RBO_recovery == 0 & CorrectResponse == 0 ~ 1L,
      RBO_recovery == 0 & CorrectResponse == 1 ~ 0L,
      TRUE ~ NA_integer_
    )
  )
...
if (!all(data$choice %in% c(1, 2))) {
  stop("Choice coding must contain only 1 = Blue and 2 = Red.")
}

if (!all(data$feedback %in% c(0L, 1L))) {
  stop("Feedback coding must contain only 1 = Blue-correct and 0 = Red-correct.")
}
...
#
# CHANGED FROM SIMPLE SCRIPT:
# This recovery script is for the Implicit Aware Base Rate group.
#
# The grid values are taken directly from param_group_global_eta_aware_exp11.csv.
# Each range is q2.5, posterior mean, q97.5 from the fitted global-eta model:
#
#   alpha  = 2.6105562 [1.6370368, 3.7184372]
#   beta   = 0.3412604 [0.1740596, 0.5060431]
#   lambda = 0.0574026 [0.0284014, 0.0890891]
...
#   lower 95% credible limit, posterior mean, upper 95% credible limit
#
# This gives:
#   3 alpha × 3 beta × 3 lambda × 3 delta × 3 eta = 243 runs
#
...
alpha_range  <- c(1.6370368025, 2.61055622215, 3.718437215)
beta_range   <- c(0.1740596355, 0.34126041495625, 0.50604311675)
lambda_range <- c(0.0284014348, 0.057402580566, 0.0890890673)
delta_range  <- c(0.61818559775, 0.8665814937375, 1.09383138)
eta_range    <- c(-0.7318049945, -0.345012521187407, 0.052685556175)
...
#   recency via lambda
#   global belief-dependent weighting of combined samples via eta
...
for (n in seq_len(N)) {
  ...
  for (j in seq_len(Tsubj[n])) {
    ...
    # Match log_trunc_global_eta_model.stan exactly:
    #   1. Combine all reliability- and recency-weighted samples.
    #   2. Determine whether their net direction confirms the current belief.
    #   3. Apply eta once to the complete sample evidence.
    #   4. Add the learned prior afterwards, without scaling the prior by eta.
    sample_evidence <- c(0.0, 0.0)
    sample_size <- sample_arr[n, j]
    ...
    for (s in seq_len(sample_size)) {
      ...
      sample_evidence[c_s] <-
        sample_evidence[c_s] + recency_weight * m_s
    }
    
    sample_log_odds <- sample_evidence[1] - sample_evidence[2]
    
    if (sample_log_odds > 0) {
      global_alignment <- 2 * V_b_clamped - 1
    } else if (sample_log_odds < 0) {
      global_alignment <- 2 * (1 - V_b_clamped) - 1
    } else {
      global_alignment <- 0
    }
    
    global_kappa <- exp(eta_n * global_alignment)
    evidence <- sample_evidence * global_kappa
    
    prior_log_odds <- log(V_b_clamped / (1 - V_b_clamped))
    evidence[1] <- evidence[1] + prior_log_odds
    
    ## Clamp and softmax exactly as in Stan; no temperature parameter.
...
results <- list(
  k                  = k,
  model_variant      = "global_eta_aware_exp11",
  stan_file           = stan_file,

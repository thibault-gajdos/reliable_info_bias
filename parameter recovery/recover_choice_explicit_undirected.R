...
# This script is for the Explicit Undirected Base Rate group.
# The grid values are taken directly from the Explicit Undirected posterior summary:
#
#   alpha  = 2.90 [1.62, 4.21]
#   beta   = 0.553 [0.284, 0.808]
#   lambda = 0.0814 [0.0517, 0.115]
#   delta  = 0.988 [0.881, 1.12]
#   eta    = -0.300 [-0.427, -0.172]
#
...
#
# This learning model uses the Explicit Undirected prior-belief data because it needs
...
load(file = './data/data_priorbelief_aware_exp12.rdata')

# Exp12 stores the response-button-order column under a manipulation-specific name.
# Standardise the name so the existing choice-recoding code works.
...
#
# CHANGED FROM SIMPLE SCRIPT:
# This recovery script is for the Explicit Undirected Base Rate group.
#
# The grid values are taken directly from the Explicit Undirected posterior summary:
#
#   alpha  = 2.90 [1.62, 4.21]
...
alpha_range  <- c(1.62, 2.90, 4.21)
beta_range   <- c(0.284, 0.553, 0.808)
lambda_range <- c(0.0517, 0.0814, 0.115)
delta_range  <- c(0.881, 0.988, 1.12)
eta_range    <- c(-0.427, -0.300, -0.172)
...
dir.create('./results/choice_learning_explicit_undirected', recursive = TRUE, showWarnings = FALSE)

out_file <- sprintf('./results/choice_learning_explicit_undirected/recover_%d.rds', k)

...
# This script is for the Explicit Deceptive Base Rate group.
# The grid values are taken directly from the Explicit Deceptive posterior summary:
#
#   alpha  = 2.47 [1.57, 3.47]
#   beta   = 0.670 [0.394, 0.947]
#   lambda = 0.0743 [0.0182, 0.164]
#   delta  = 1.08 [0.906, 1.30]
#   eta    = -0.219 [-0.382, -0.0696]
#
...
#
# This learning model uses the Explicit Deceptive prior-belief data because it needs
...
load(file = './data/data_priorbelief_deceptive_exp13.rdata')

# Some preprocessed task files store the response-button-order column under a
# manipulation-specific name. Standardise it so the existing recoding code works.
if (!"ResponseButtonOrder" %in% names(data) && "Manipulation_ResponseButtonOrder" %in% names(data)) {
  data$ResponseButtonOrder <- data$Manipulation_ResponseButtonOrder
}
...
#
# CHANGED FROM SIMPLE SCRIPT:
# This recovery script is for the Explicit Deceptive Base Rate group.
#
# The grid values are taken directly from the Explicit Deceptive posterior summary:
#
#   alpha  = 2.47 [1.57, 3.47]
...
alpha_range  <- c(1.57, 2.47, 3.47)
beta_range   <- c(0.394, 0.670, 0.947)
lambda_range <- c(0.0182, 0.0743, 0.164)
delta_range  <- c(0.906, 1.08, 1.30)
eta_range    <- c(-0.382, -0.219, -0.0696)
...
dir.create('./results/choice_learning_explicit_deceptive', recursive = TRUE, showWarnings = FALSE)

out_file <- sprintf('./results/choice_learning_explicit_deceptive/recover_%d.rds', k)
...
# Avoid summarising the entire Stan object, which can be very memory-heavy.
# Diagnostics are calculated only over the recovered group and individual
# parameters that are actually used in the recovery plots.
diagnostic_params <- bind_rows(group_fitted, indiv_fitted)
...
results <- list(
  ...
  max_rhat           = max(diagnostic_params$rhat, na.rm = TRUE),
  min_ess            = min(diagnostic_params$ess_bulk, na.rm = TRUE)
)

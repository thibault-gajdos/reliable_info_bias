...
# This script is for the Explicit True Base Rate group.
# The grid values are taken directly from the Explicit True posterior summary:
#
#   alpha  = 4.12 [2.95, 5.25]
#   beta   = 0.405 [0.204, 0.608]
#   lambda = 0.0431 [0.0191, 0.0701]
#   delta  = 1.45 [1.18, 1.78]
#   eta    = -0.110 [-0.480, 0.278]
#
...
#
# This learning model uses the Explicit True prior-belief data because it needs
...
load(file = './data/data_priorbelief_truthful_exp13.rdata')
...
#
# CHANGED FROM SIMPLE SCRIPT:
# This recovery script is for the Explicit True Base Rate group.
#
# The grid values are taken directly from the Explicit True posterior summary:
#
#   alpha  = 4.12 [2.95, 5.25]
...
alpha_range  <- c(2.95, 4.12, 5.25)
beta_range   <- c(0.204, 0.405, 0.608)
lambda_range <- c(0.0191, 0.0431, 0.0701)
delta_range  <- c(1.18, 1.45, 1.78)
eta_range    <- c(-0.480, -0.110, 0.278)
...
dir.create('./results/choice_learning_explicit_true', recursive = TRUE, showWarnings = FALSE)

out_file <- sprintf('./results/choice_learning_explicit_true/recover_%d.rds', k)
